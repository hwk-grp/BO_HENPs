...
# read data
# 1st round
# df0 = pd.read_excel('data/metal8_HER_50.xlsx')
...
for repeat in range(0, n_repeat):
    ...
    acq_function = UtilityFunction(kind="ucb", kappa=1)
    ...
    # print(optimizer)
    x_max = list()
    x_max_raw = optimizer.suggest(acq_function)
    for i in range(0, 8):
        x_max.append(x_max_raw['x{}'.format(i+1)])
    bo_cats.extend(x_max)
    x_max = np.array(x_max).reshape(1, -1)

    target_final, std_final = model.predict(x_max, return_std=True)
    bo_cats.append(target_final.item())
    bo_cats.append(std_final.item())

bo_cats = np.array(bo_cats).reshape(-1, 10)
df1 = pd.DataFrame(bo_cats,
                   columns=list(df0.columns) + ['uncertainty'])
# df1.to_excel("suggested_cats_HER_1st.xlsx", index=False)
df1.to_excel("suggested_cats_HER_2nd.v2_acq_func.wo_max.xlsx", index=False)